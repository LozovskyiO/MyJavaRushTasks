...
public class Solution {
    ...
    public static void main(String[] args) throws Exception {
        String phrase = new BufferedReader(new InputStreamReader(System.in)).readLine();
        ArrayList<Character> vowelArray = new ArrayList<>();
        ArrayList<Character> consonantlArray = new ArrayList<>();


        for (int i = 0; i < phrase.length(); i++) {
           char tempChar = phrase.charAt(i);

           if (isVowel(tempChar)&&(tempChar!=' ')){
                vowelArray.add(tempChar);
            } else if (tempChar!=' ')
            {consonantlArray.add(tempChar);}
